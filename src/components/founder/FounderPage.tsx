...
const timeline = [
  ...
  { year: '2025', fr: 'Fondation de Gëstu Lab — structuration de l\'activité', en: 'Foundation of Gëstu Lab — structuring the activity' },
];
  ...
  return (
    <>
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-dark grid-pattern pt-28 pb-20">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 grid md:grid-cols-[1fr_340px] gap-16 items-center w-full">
          <div>
            <Link href="/" className="inline-flex items-center gap-2 text-text-muted text-sm hover:text-white transition-colors mb-10">
              ...
              {t('Retour à Gëstu Lab', 'Back to Gëstu Lab')}
            </Link>

            <p className="text-indigo-light text-xs font-bold uppercase tracking-widest mb-4">{t('Le fondateur', 'The founder')}</p>
            ...
            <p className="gradient-text text-xl font-bold mb-8">
              {t('Fondateur & Lead Developer — Gëstu Lab', 'Founder & Lead Developer — Gëstu Lab')}
            </p>
            <p className="text-text-secondary text-lg leading-relaxed max-w-xl mb-10">
              {t(
                'Développeur full-stack, designer UI/UX et expert en systèmes d\'information. 3+ ans à construire des solutions numériques qui transforment des organisations.',
                'Full-stack developer, UI/UX designer and information systems expert. 3+ years building digital solutions that transform organizations.'
              )}
            </p>
            <div className="flex flex-wrap gap-2 mb-10">
              {['🇫🇷 Français', '🇬🇧 English', '🇸🇳 Wolof'].map(l => (
                <span key={l} className="text-xs font-medium bg-white/5 border border-white/10 text-text-secondary px-3 py-1.5 rounded-full">{l}</span>
              ))}
            </div>
            <div className="flex gap-4 flex-wrap">
              ...
              <a href="https://linkedin.com/in/el-hadji-abdou-babou-ngom" target="_blank" rel="noopener" className="px-7 py-3.5 rounded-xl font-bold border border-white/10 text-white hover:bg-white/5 transition-all">
                LinkedIn
              </a>
            </div>
          </div>
          ...
          <div className="hidden md:flex flex-col items-center">
            <div className="w-64 h-80 rounded-2xl bg-gradient-to-br from-indigo/30 to-cyan/20 border border-white/10 flex flex-col items-center justify-center gap-4 overflow-hidden glow-indigo">
              <svg viewBox="0 0 80 80" fill="none" className="w-28 h-28">
                <circle cx="40" cy="30" r="20" fill="#6366F1" opacity="0.8"/>
                <path d="M5 80c0-19.33 15.67-35 35-35s35 15.67 35 35" fill="#06B6D4" opacity="0.6"/>
              </svg>
              <p className="text-text-muted text-xs text-center px-4">{t('Ajoutez votre photo ici', 'Add your photo here')}</p>
            </div>
            <div className="mt-4 flex gap-4 text-center">
              <div><div className="font-black text-2xl gradient-text">3+</div><div className="text-text-muted text-xs">{t('ans', 'years')}</div></div>
              <div><div className="font-black text-2xl gradient-text">8+</div><div className="text-text-muted text-xs">{t('projets', 'projects')}</div></div>
              <div><div className="font-black text-2xl gradient-text">5+</div><div className="text-text-muted text-xs">{t('secteurs', 'sectors')}</div></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-28 bg-dark-800">
        <div className="max-w-4xl mx-auto px-6">
          <p className="text-cyan text-xs font-bold uppercase tracking-widest mb-8 text-center">{t('Mot du Fondateur', "Founder's Word")}</p>
          <div className="glass gradient-border rounded-3xl p-10 md:p-14 relative">
            ...
            <blockquote className="relative z-10">
              ...
              <p className="text-text-secondary text-base leading-relaxed mb-6">
                {t(
                  "J'ai fondé Gëstu Lab avec une conviction : les organisations africaines méritent des solutions numériques de classe mondiale, pensées pour leur réalité. Pas des outils génériques importés. Des solutions ancrées dans nos contextes, nos contraintes, notre terrain.",
                  "I founded Gëstu Lab with one conviction: African organizations deserve world-class digital solutions built for their reality. Not generic imported tools. Solutions rooted in our contexts, our constraints, our ground."
                )}
              </p>
              <p className="text-text-secondary text-base leading-relaxed mb-10">
                {t(
                  "Aujourd'hui je suis seul. Demain nous serons une équipe. Mais la philosophie ne changera jamais : explorer avec rigueur, construire avec intégrité, livrer avec ambition. Si vous lisez ces lignes, c'est que vous cherchez un partenaire sérieux pour votre transformation numérique. Je ne vous promets pas le prix le plus bas. Je vous promets de comprendre votre métier avant d'écrire la première ligne de code.",
                  "Today I work alone. Tomorrow we will be a team. But the philosophy will never change: explore with rigor, build with integrity, deliver with ambition. If you're reading this, you're looking for a serious partner for your digital transformation. I don't promise the lowest price. I promise to understand your business before writing the first line of code."
                )}
              </p>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo/40 to-cyan/30 border border-white/10 flex items-center justify-center">
                  <span className="text-white font-black text-lg">E</span>
                </div>
                <div>
                  <div className="text-white font-bold">El Hadji Abdou Babou Ngom</div>
                  <div className="text-text-muted text-sm">{t('Fondateur, Gëstu Lab', 'Founder, Gëstu Lab')}</div>
                </div>
              </div>
            </blockquote>
          </div>
      ...
      <section className="py-28 bg-dark">
        <div className="max-w-4xl mx-auto px-6">
          <p className="text-indigo-light text-xs font-bold uppercase tracking-widest mb-4 text-center">{t('Mon parcours', 'My journey')}</p>
          <h2 className="text-[clamp(1.8rem,4vw,2.8rem)] font-black text-white text-center mb-16">{t('5 ans de construction', '5 years of building')}</h2>
      ...
      <section className="py-28 bg-dark-800">
        <div className="max-w-6xl mx-auto px-6">
          <p className="text-cyan text-xs font-bold uppercase tracking-widest mb-4 text-center">{t('Expertise technique', 'Technical expertise')}</p>
            ...
            {skills.map((g, i) => (
              <div key={i} className="glass gradient-border rounded-xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {g.tags.map(tag => (
                    <span key={tag} className="text-xs font-semibold bg-white/5 border border-white/8 text-text-secondary hover:text-white hover:border-indigo/40 px-2.5 py-1.5 rounded-lg transition-all cursor-default">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      {/* ── POURQUOI TRAVAILLER AVEC GËSTU LAB ── */}
      <section className="py-28 bg-dark dot-pattern">
        <div className="max-w-6xl mx-auto px-6">
          <p className="text-indigo-light text-xs font-bold uppercase tracking-widest mb-4 text-center">{t('La différence', 'The difference')}</p>
          <h2 className="text-[clamp(1.8rem,4vw,2.8rem)] font-black text-white text-center mb-4">
            {t('Pourquoi choisir', 'Why choose')}{' '}
            <span className="gradient-text">Gëstu Lab</span> ?
          </h2>
      ...
      <section className="py-28 bg-dark-800 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto px-6 text-center">
          ...
          <p className="text-text-secondary text-lg mb-10">
            {t(
              'Un projet en tête ? Discutons-en. Je vous réponds dans les 24 heures.',
              'A project in mind? Let\'s talk. I\'ll get back to you within 24 hours.'
            )}
          </p>
          <a href="mailto:ngomabdoubabou@outlook.fr" className="btn-gradient px-10 py-4 rounded-xl font-bold text-lg inline-block">
